...
class UserSerializer(serializers.Serializer):
    ...
    def validate_username(self, data):
        ...
        # Si esroy actualizando, el nuevo username es diferente al de la instancia (está cambiado el usuario)
        # y existen usuarios ya registrados con el nuevo username
        elif self.instance is not None and self.instance.username != data and len(users) != 0:
            raise serializers.ValidationError("Estas cambiando el nombre de usuario por otro que ya existe")
        else:
            return data
